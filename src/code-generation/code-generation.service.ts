...
export class CodeGenerationService {
  ...
  private validateAndNormalizeDiagram(classes: any[]) {
    ...
    for (const umlClass of classes) {
      ...
      const existingIdAttr = umlClass.attributes.find((attr: any) => 
        attr.stereotype === 'id' || attr.name?.toLowerCase() === 'id'
      );
  ...
  private transformAttributes(attributes: any[], classId: string, className: string, relations: any[], allClasses: any[], parentClass?: any) {
    ...
    for (const attr of attributes) {
      const isId = attr.stereotype === 'id' || (attr.name?.toLowerCase() === 'id' && !idHandled);
      ...
      // If we already handled an ID attribute and this is another attribute named 'id', skip it
      if (!isId && attr.name?.toLowerCase() === 'id') {
        console.log(`⏭️  Skipping duplicate id attribute: ${attr.name}`);
        continue;
      }
